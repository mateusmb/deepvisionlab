...
# Function that find intersection point of two lines
def intersect(line1, line2):
    delta = np.array([line1[1] - line1[0], line2[1] - line2[0]]).astype(np.float32)

    delta = 1 / delta
    delta[:, 0] *= -1

    b = np.matmul(delta, np.array((line1[0], line2[0])).transpose())
    b = np.diagonal(b).astype(np.float32)

    res = cv2.solve(delta, b)
    return res[0], tuple(res[1].astype(np.int32).reshape((2)))

# Define function to un-warp perspective distortions
# Calculates the correspondence between four pairs of distorted and
# undistorted points
def rectify(image, corners, out_size):
    ...
    dst = np.array([[0,0], [out_size[1] - 1, 0],
...
# Function to find the outer corners of the QR
def qr_code_outer_corners(image):
    ...
    if len(centers) == 3:
        distance_between_pts = np.linalg.norm(np.roll(centers, 1, 0) - centers, axis=1)
        max_dist_id = np.argmax(distance_between_pts)
...
while True:
    ...
    # Search for QR Code in each frame
